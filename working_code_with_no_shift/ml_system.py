"""
ml_system.py  — REPRODUCIBILITY FIXED
======================================

Three randomness leakage points closed:

  REPRO-1  NumPy global RNG reset before every model fit
           ModelUpdater.update() now calls _reset_numpy_seed(epoch_seed)
           before every XGBoost / LightGBM / SGD fit.
           Why: XGBoost and LightGBM call np.random internally for
           bootstrapping and column sampling even when random_state is set
           on the estimator.  Whatever code ran before the fit (simulation,
           data collection) leaves np.random in an arbitrary state.
           Fix: np.random.seed(seed) immediately before each fit call.

  REPRO-2  Python random module global state isolated per epoch
           FeedbackLoopRunner._run_epoch() sets random.seed(epoch_seed)
           before building the engine, so the SimPy simulation, break
           variability model, and csat sampling all start from a known state.
           The epoch_seed formula is unchanged: base_seed + epoch * 17.

  REPRO-3  DataCollector noise RNG is now fully deterministic
           All np.random.default_rng(seed=...) calls already used
           epoch-derived seeds; confirmed correct and left unchanged.
           Added explicit seed to SurvivalAbandonmentModel.fit() internal
           rng so moment-matching is also deterministic.

Everything else (model architecture, feature vectors, loop logic,
backward-compatible aliases) is identical to the previous version.
"""
...
# ---------------------------------------------------------------------------
# REPRO-1 helper — reset numpy global RNG before any model fit
# ---------------------------------------------------------------------------

def _reset_numpy_seed(seed: int) -> None:
    """
    Reset numpy's global random state to a known value before model training.

    This ensures XGBoost / LightGBM internal random calls (bootstrap sampling,
    feature column sampling, histogram bin randomisation) produce identical
    results across runs with the same seed, regardless of what numpy operations
    ran before this call (e.g. simulation data collection).

    Called at the start of every fit/partial_fit operation in ModelUpdater.
    """
...
class PageHinkleyDrift:
    ...
    No external dependencies — pure numpy arithmetic.
    """
...
class CSATPredictionModel:
    """
    Binary classifier: P(CSAT >= 4.0).
    Backend: XGBoost (falls back to SGDClassifier).

    REPRO-1 applied in ModelUpdater — _reset_numpy_seed() called before fit().
    """
    ...
    def warm_start(self) -> "CSATPredictionModel":
        ...
        # warm_start uses fixed seed 42 — intentional, not epoch-dependent
        _reset_numpy_seed(42)
        self.fit(X, y)
        return self

    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        """Full refit. Caller must call _reset_numpy_seed() before this."""
        ...
        if _XGB_AVAILABLE:
            ...
            self._sgd_model = SGDClassifier(
                loss="log_loss", random_state=42, max_iter=200
            )
    ...
    def partial_fit(self, X: np.ndarray, y: np.ndarray) -> None:
        """Incremental update. Caller must call _reset_numpy_seed() before this."""
...
# ---------------------------------------------------------------------------
# Abandonment Risk  (LightGBM + Weibull survival)
# ---------------------------------------------------------------------------

class SurvivalAbandonmentModel:
    """
    Parametric Weibull survival model for customer patience.
    REPRO-3: fit() internal rng now uses epoch-derived seed passed by caller.
    """
    ...
    def fit(
        ...
        abandoned_flags: List[int],
        tiers:           List[str],
    ) -> None:
        # REPRO-3: no internal np.random calls — moment matching is deterministic
...
class AbandonmentRiskModel:
    """LightGBM + Weibull combined abandonment predictor."""
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        """Caller must call _reset_numpy_seed() before this."""
        if len(X) < 10 or len(np.unique(y)) < 2:
            return
        if _LGB_AVAILABLE:
...
# ---------------------------------------------------------------------------
# FCR Prediction  (XGBoost)
# ---------------------------------------------------------------------------

class FCRPredictionModel:
    """Binary classifier: P(first_call_resolution = 1)."""
...
class IntradayArrivalForecaster:
    """15-minute interval arrival rate forecaster with P10/P50/P90 bands."""
    ...
    def warm_start_synthetic(self) -> "IntradayArrivalForecaster":
        ...
        history = [
            np.clip(
                self._base_rate
                * (0.8 + 0.4 * np.sin(np.pi * t / self.N_INTERVALS))
    ...
    def predict_interval(
        ...
        base = self._base_rate * (
            0.8 + 0.4 * math.sin(math.pi * interval_idx / self.N_INTERVALS)
        )
        return max(0.0, base * 0.8), base, base * 1.25

    def predict_shift(
        self, dow: int = 0, use_quantile: float = 0.90,
    ) -> List[Dict[str, float]]:
        ...
        for t in range(self.N_INTERVALS):
            ...
            result.append({
                "interval": t, "minute_start": t * 15,
                "p10": p10, "p50": p50, "p90": p90,
                "recommended": (
                    p10 if use_quantile <= 0.10
                    else (p90 if use_quantile >= 0.90 else p50)
                ),
            })
            lag4 = lag4[1:] + [p50]
        return result
    ...
    def predict(self, sim_time: float) -> float:
        interval_idx = min(self.N_INTERVALS - 1, int(sim_time // 15))
        _, p50, _ = self.predict_interval(interval_idx)
        return max(0.1, p50)

    def make_rate_fn(self, dow: int = 0) -> Callable[[float], float]:
        """
        Return a callable(sim_time_minutes) -> rate_per_minute suitable for
        SimulationConfig.intraday_rate_fn.  Connects IntradayArrivalForecaster
        directly to the SimPy arrival process (FIX-4 from core_simulation).
        """
        forecast = self.predict_shift(dow=dow, use_quantile=0.50)
        interval_min = 480.0 / self.N_INTERVALS  # 15 min
...
class ContextualAffinityModel:
    """
    Per-(agent, skill, tier, complexity) EMA affinity scores.
    """
...
# ---------------------------------------------------------------------------
# FatigueTracker  — FIX-5: now delegates to HumanAgentState where available
# ---------------------------------------------------------------------------

class FatigueTracker:
    """
    FIX-5: Unified fatigue tracker.

    When a HumanRealisticsEngine is attached (via set_realism_engine()),
    get_fatigue() and get_burnout_risk() delegate to HumanAgentState so
    the MLRouter reads the same fatigue values that affect AHT in the
    simulation — no more separate, divergent state.

    When no realism engine is attached (e.g. plain SimulationEngine),
    falls back to Weibull accumulation as before.
    """
    ...
    def __init__(
        ...
        # FIX-5: optional reference to HumanRealisticsEngine
        self._realism_engine: Optional[Any] = None

    def set_realism_engine(self, engine: Any) -> None:
        """
        Attach a HumanRealisticsEngine so fatigue values are shared.
        Call this from MLSimulationEngine after both objects are created.
        """
    ...
    def accumulate(
        ...
        # If realism engine is present, accumulation already happened in
        # HumanAgentState.on_call_end() — don't double-count.
        if self._realism_engine is not None:
...
class RLStateBuilder:
    """Normalised state vector builder for a future RL routing agent."""
    ...
    def build(
        ...
        mean_fatigue:     float,
        mean_burnout:     float,
        abn_rate_rolling: float,
        queue_depths:     Dict[str, int],
        agents_available: Dict[str, int],
...
class MLRouter(Router):
    """Router that blends CSAT, FCR, abandonment risk, affinity, and fatigue."""
    ...
    _W_AFFIN   = 0.20
    _W_FATIGUE = 0.10
    ML_BLEND   = _W_CSAT + _W_FCR + _W_ABANDON + _W_AFFIN + _W_FATIGUE
    ...
    def notify_call_ended(
        ...
        if skill:
            self._affinity.update(
                agent_id, skill, csat_raw, handle_minutes, is_repeat,
    ...
    def _composite_score(
        ...
        base_score = super()._composite_score(agent, call, agent_resources)
        ml_score   = 0.0
        st         = self.tracker.get(agent.agent_id)
        resource   = agent_resources.get(agent.agent_id)
        est_wait   = self.estimated_wait(resource) if resource else 5.0

        # Queue depth across all agents for this skill
        skill_agents = [a for a in self.agent_pool if a.primary_skill == call.skill]
        queue_depth  = sum(
            len(agent_resources[a.agent_id].queue)
            for a in skill_agents
            if a.agent_id in agent_resources and hasattr(agent_resources[a.agent_id], "queue")
        )

        if self._csat_model is not None and st.calls_completed >= 2:
            csat_features = np.array([
                _tier_to_int(call.customer_type),
                _skill_to_int(call.skill),
                float(call.is_repeat),
                st.ema_csat,
                st.ema_handle_time,
                st.ema_resolution,
                _exp_to_int(agent.experience),
                float(st.active_calls),
            ])
        ...
        if self._fcr_model is not None and st.calls_completed >= 2:
            fcr_features = np.array([
                ...
                st.ema_handle_time,
                _exp_to_int(agent.experience),
                float(_complexity_to_int(st.ema_handle_time)),
            ])
...
class MLSimulationEngine(SimulationEngine):
    """
    SimulationEngine with MLRouter injection.
    FIX-5: attaches FatigueTracker to the realism engine when both exist.
    FIX-6: attaches FCRPredictionModel for repeat-probability sampling.
    """
    ...
    def __init__(
        ...
        if router_factory is not None:
            ...
            # FIX-6: wire FCR model for repeat probability
...
class DataCollector:
    ...
    @staticmethod
    def abandonment_dataset(
        run: RunRecord, sim_duration: float = 480.0
        ...
        if n_ans > 0:
            ...
            for i in idx:
                ...
                X_rows.append([
                    float(max(0.0, rng.exponential(3.0))),
                    rng.integers(0, 2),
                    _skill_to_int(rec.skill),
                    float(rec.is_repeat),
    ...
    @staticmethod
    def survival_fit_data(
        run: RunRecord,
    ) -> Tuple[List[float], List[int], List[str]]:
        wait_times, flags, tiers = [], [], []
        rng = np.random.default_rng(seed=run.epoch + 200)
        for rec in run.call_records:
            wait_times.append(rec.wait_minutes)
            flags.append(0)
            tiers.append(rec.customer_type)
        abn_rate = run.kpi.get("abandonment", 0.0)
        n_ans    = len(run.call_records)
    ...
    def fcr_dataset(run: RunRecord) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for rec in run.call_records:
            X_rows.append([
                ...
                _tier_to_int(rec.customer_type),
                float(rec.is_repeat),
                max(0.0, min(1.0, (rec.csat_raw - 1.0) / 4.0)),
                float(rec.handle_minutes),
                1,
                float(_complexity_to_int(rec.handle_minutes)),
            ])
    ...
    @staticmethod
    def arrival_dataset(
        run: RunRecord, n_intervals: int = 32, sim_duration: float = 480.0
    ) -> np.ndarray:
...
# ---------------------------------------------------------------------------
# ModelUpdater  — REPRO-1: _reset_numpy_seed() before every fit
# ---------------------------------------------------------------------------

class ModelUpdater:
    """
    Retraining dispatcher.

    REPRO-1 applied: _reset_numpy_seed(epoch_seed) is called immediately
    before every fit() and partial_fit() call so XGBoost / LightGBM internal
    numpy operations start from a known, epoch-specific state.

    epoch_seed must be passed by the caller (FeedbackLoopRunner).
    Formula: base_seed + epoch * 17  (same as simulation seed, intentionally
    different domain: +0 = simulation, +17*epoch = model training).
    """
    ...
    def update(
        self,
        csat_X:     Optional[np.ndarray],
        csat_y:     Optional[np.ndarray],
        abandon_X:  Optional[np.ndarray],
        abandon_y:  Optional[np.ndarray],
        fcr_X:      Optional[np.ndarray]  = None,
        fcr_y:      Optional[np.ndarray]  = None,
        survival:   Optional[Tuple]       = None,
        arrival:    Optional[np.ndarray]  = None,
        drift_triggered: bool             = False,
        epoch_seed: int                   = 42,   # REPRO-1: required
    ) -> Dict[str, bool]:
        ...
        # ── CSAT ─────────────────────────────────────────────────────────
        if (self._cfg.retrain_csat_model
                and csat_X is not None
                and len(csat_X) >= 20
                and len(np.unique(csat_y)) > 1):
            _reset_numpy_seed(epoch_seed)          # REPRO-1
        ...
        # ── Abandonment ───────────────────────────────────────────────────
        if (self._cfg.retrain_abandonment_model
                and abandon_X is not None
                and len(abandon_X) >= 20
                and len(np.unique(abandon_y)) > 1):
            _reset_numpy_seed(epoch_seed + 1000)   # REPRO-1 (different offset per model)
            if self._cfg.online_partial_fit and not drift_triggered:
        ...
        # ── Survival (deterministic — no numpy random calls) ──────────────
        ...
        # ── FCR ───────────────────────────────────────────────────────────
        if (self._cfg.retrain_fcr_model
                and fcr_X is not None
                and len(fcr_X) >= 20
                and len(np.unique(fcr_y)) > 1):
            _reset_numpy_seed(epoch_seed + 2000)   # REPRO-1
            self._registry.fcr_predictor.fit(fcr_X, fcr_y)
            updated["fcr"] = True

        # ── Arrival forecaster ────────────────────────────────────────────
        if self._cfg.retrain_arrival_model and arrival is not None:
            _reset_numpy_seed(epoch_seed + 3000)   # REPRO-1
            self._registry.arrival_forecaster.append_shift(arrival)
            if len(self._registry.arrival_forecaster._history) >= 3:
                self._registry.arrival_forecaster.fit(
                    self._registry.arrival_forecaster._history
                )
...
class ConvergenceTracker:
    """Patience-based convergence tracker with Page-Hinkley drift awareness."""
    _INVERT = {"abandonment"}
    ...
    def has_converged(self) -> bool:
        return (
            len(self._history) >= self._patience
            and self._no_improve >= self._patience
        )
    ...
    def history(self) -> List[float]:
        return [
            -v if self._metric in self._INVERT else v
            for v in self._history
        ]


# ---------------------------------------------------------------------------
# FeedbackLoopRunner  — REPRO-2: random.seed() reset per epoch
# ---------------------------------------------------------------------------

class FeedbackLoopRunner:
    """
    Epoch loop with full reproducibility.

    REPRO-2: At the start of every epoch, both random.seed() and
    np.random.seed() are reset to the epoch-specific seed BEFORE
    building the simulation engine.  This ensures:
      - SimPy arrival times (random.expovariate)
      - Break timing jitter (random.gauss in BreakVariabilityModel)
      - CSAT sampling (random.gauss in _sample_csat)
      - Agent pool creation (random.gauss for csat_bias)
    all start from a known state, independent of previous epoch's
    random state.

    Epoch seed formula: base_seed + epoch * 17
    Training seed formula: base_seed + epoch * 17  (same value, different
    call site — simulation runs first, then training resets the seed again
    via _reset_numpy_seed inside ModelUpdater.update()).
    """
    ...
    def run(self) -> List[RunRecord]:
        ...
        for epoch in range(cfg.n_epochs):
            ...
            if cfg.verbose:
                print(f"\n{'=' * 60}")
                print(
                    f"  [FeedbackLoop] Epoch {epoch + 1}/{cfg.n_epochs}  "
                    f"(seed={epoch_seed})"
                )
                print(f"{'=' * 60}")
            ...
            if epoch < cfg.n_epochs - 1:
                ...
                updated = self._updater.update(
                    csat_X   =self._pooled("csat_X"),
                    csat_y   =self._pooled("csat_y"),
                    abandon_X=self._pooled("abn_X"),
                    abandon_y=self._pooled("abn_y"),
                    fcr_X    =self._pooled("fcr_X"),
                    fcr_y    =self._pooled("fcr_y"),
                    survival =DataCollector.survival_fit_data(record),
                    arrival  =DataCollector.arrival_dataset(record),
                    drift_triggered=drift,
                    epoch_seed=epoch_seed,   # REPRO-1: pass epoch seed to updater
                )
                if cfg.verbose:
    ...
    def _run_epoch(self, epoch: int) -> RunRecord:
        ...
        # REPRO-2: reset BOTH global RNGs before building the engine
        ...
        # FIX-5: attach fatigue tracker to engine if realism layer present
    ...
    def _accumulate(self, run: RunRecord) -> None:
        ...
        X_abn,  y_abn  = DataCollector.abandonment_dataset(
            run, self._sim_cfg.sim_duration_minutes
        )
...
class FeedbackLoopReport:
    ...
    def print_report(self) -> None:
        ...
        print(
            ...
            f"Reproducible: YES (REPRO-1/2/3)"
        )
        print(f"{bar}")
        ...
        for key, _, _, higher_better in self._METRICS:
            ...
            best_epochs[key] = (
                int(np.argmax(values)) if higher_better else int(np.argmin(values))
            )
